#v2: incorporate reformatted appendix figures, code from 3-11-19
...
ggplot(data = rrc_hin, aes(x = k, y = Coef)) + 
  #geom_pointrange(aes(ymin = lo, ymax = hi, x = k, color = Seg, alpha = sig), 
  geom_pointrange(aes(ymin = lo, ymax = hi, x = k, shape = Seg, alpha = sig), 
                  position = position_dodge(width = 0.9)) +
  scale_shape_manual('Exposure', values = c(16, 17), labels = c('Low','High')) + #Changed 3-7-19 to swap high/low labels
  scale_alpha_manual('p < 0.05', c(TRUE, FALSE), values=c(0.5, 1), labels = c('Yes','No')) +
  #labs(shape = 'Segregation') +
  labs(shape = 'Exposure') + #this changed 3-7-19
  theme_minimal() + ylab('Coethnicity Coefficients') +
  theme(text = element_text(size = 16),
        plot.title = element_text(hjust = 0.5)) + 
  ggtitle('Nearest-k Own Religion (Hindu)') +
  guides(shape = guide_legend(order = 1), #why does it go in reverse order
         alpha = guide_legend(order = 0))
...
rrc_mus = 
  ...
  gather(Seg, Coef, c(Low, High)) %>% mutate(Coef = as.numeric(Coef),
                                             ...
                                             lo = Coef - 1.96*sd,
                                             hi = Coef + 1.96*sd,
                                             sig = factor(p_a2 < 0.05))#choose appropriate sd's
#FIGURE A7
